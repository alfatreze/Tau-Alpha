...
| Hardware capacity | Uncached SDRAM CPU data window | A-066 **Pocket** proves `sdram_fb` writes and reads `FFFF`. A-067's delayed bridge capture regressed the previously passing MMIO preflight and is rejected. A-074/A-075 prove bridge `FFFFFFFF`; A-076 proves CPU zero; A-077 proves adapter zero; A-079 **Pocket** proves mux zero at completion (`G-G-R`). A-092 showed writes land and reads lag one beat back-to-back; A-093 traced this to the Wishbone adapter re-accepting each completed beat (reproduced in simulation, fixed with a second release cycle) and **Pocket** PASSes the 183-check matrix with 0 failures. Uncached data path only: cached access, contention stress and cold-data migration remain unauthorised. | [issues 016](issues/016-phase2-cpu-window-first-transaction-stall.md), [017](issues/017-a064-probe-stimulus-mismatch.md), and [018](issues/018-phase2-post-bridge-write-readback.md) |
