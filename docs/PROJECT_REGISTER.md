...
| [018](issues/018-phase2-post-bridge-write-readback.md) | A-065 reaches the SDRAM-domain bridge with `FFFFFFFF` and both controller requests accepted, but later reads still return zero. | Open; A-075 proves bridge assembly is `FFFFFFFF`; A-076 CPU return, A-077 adapter return, and A-079 mux return are all zero. Reproduce bridge system-response → mux timing before any migration decision. |
