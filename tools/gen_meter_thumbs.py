...
The firmware draws each 56x32 preview from a 16-colour RGB565 palette and a raster
run-length stream (one byte per run: high nibble = palette index, low nibble = run length - 1,
so 1..16 pixels; runs may cross row ends). This keeps all eleven previews to a few KiB of ROM,
...
def runs(idx):
    ...
    while i < len(flat):
        n = 1
        while i + n < len(flat) and flat[i + n] == flat[i] and n < 16:
            n += 1
