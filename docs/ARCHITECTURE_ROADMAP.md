...
| Decoder stage cost (Phase D step 1) | **[HW, B-087]** MP3 measured, prediction wrong: **Subband (the synthesis filterbank) dominates, not the bit reader.** Three bitrates, `TAU DEV 36`: 128 kbps CBR 44.1k H3/I11-12/S55 (~69-70% of realtime); 320 kbps CBR 44.1k H5/I13/S55 (~73%); 320 kbps CBR 48k H5/I13/S59 (~77%). Huffman/bit-reading (H) is the *smallest* share at 3-5%, confirmed unchanged whether an accidental 1.2x speed mode was engaged or not (retested) — the speed feature resamples already-decoded PCM downstream of Subband, so it doesn't touch these numbers. **FLAC's R could not be refreshed**: the `flac-tests/` synthetic vectors used (t1-t6, chosen because the original FLAC.md files are gone, see B-086) turned out to be built entirely from VERBATIM subframes (`tools/flac_make_test.py`: "every subframe VERBATIM", for decode-path correctness testing) — VERBATIM bypasses the Rice/LPC bit-reader path R measures, so R0 across all six is the *correct* reading for that content, not a bug, and not evidence about real-music cost. FLAC.md's original 64-76% R stands unrefreshed; needs a real-music FLAC file to redo. **Conclusion: the two codecs point different ways, so the kernel order is per-codec, not universal.** For MP3 the *original* roadmap order (filterbank first) is right after all — a bit-reader kernel would only touch 3-5% of decode, while the filterbank is over half; D3's inference (extrapolating FLAC's bit-reader dominance to MP3) was an overgeneralization and is corrected here. FLAC's own bit-reader-first case (from the real measurement that does exist) is untouched by this — it was never about MP3. | Kernel work (Phase D step 7 / PHASE_F_SPEC item 7) | **MP3: build the synthesis-filterbank kernel first**, matching the pre-D3 roadmap order. **FLAC: still open** — redo the R refresh on a real-music FLAC file (t1-t6 cannot answer it) before ordering FLAC's kernel. **[HW, B-098] FLAC refresh done, on the content-controlled Test Album** (real music, not synthetic vectors; a cross-format contamination bug found and fixed along the way, B-097). MacCunn (dense) R 12% (44.1k) / 15% (48k); Clementi (sparse) R 7% (44.1k) / 11% (48k) — confirms Subband/MP3's finding of "bit reader is not always dominant," but **contradicts FLAC.md's own original figure** (Pink Floyd 16-bit measured R=64%; these two pieces sit at 7-15%, nowhere near it) despite both being real, comparably-bitrated 16-bit content. Not yet explained — content/dynamics may matter far more to R than bit depth alone, or the original files had some other distinguishing property; needs the *same* files re-measured before trusting either number over the other, and neither is a stand-in for the other. MP3's own finding holds up cleanly on real content: Subband dominates (41-58%), Huffman smallest (2-5%), rising slightly with bitrate as predicted (H 3%→5% at 128→320 kbps) while IMDCT/Subband stay roughly flat — matching the *design*, not the earlier stress-test-file numbers. One new, unpredicted result: at the same 320 kbps, all three stages read *lower* at 48 kHz than 44.1k (H5/I13/S54 vs H3/I9/S41) — consistent with the same bits-per-sample logic FLAC.md found (320 kbps over more samples/sec means fewer coded bits per sample), now shown to apply to MP3 too. |
