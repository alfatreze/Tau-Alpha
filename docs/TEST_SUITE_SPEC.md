...
## 11. Decoder stage cost in the Check record (spec only, 2026-09-22, from B-086/B-087) — not built
...
**Not built — spec only (B-088). The build itself is the next free audit id after that.** Scoped as its own session so it does not entangle further with `fw/player.c`/`fw/build.sh`, which already carry unrelated uncommitted work (see B-086's commit note).
