...
**No toast.** The mode row shows the preset's NAME where the word `EQ` would
otherwise sit — same screen area, but it says which preset is on rather than
merely that the feature exists, and it is permanent rather than fading. A toast
as well would put the same word on screen twice at once.
...
### Tried and BACKED OUT: the curve over the meter

The meter band was made to show the preset's actual response for ~1 s on a
preset change, then morph back into the live meter. It was built, it worked, and
**the user did not want it** — taking the meter over is too much for a control
change, however good the picture is.

Backed out 2026-08-04. The coefficient-derived curve table is still generated by
`gen_eq_coeffs.py --curves` but sits behind `#if EQ_SHOW_CURVE` (default 0), so
reviving it is a one-line change rather than a re-derivation. The ASCII preview
...
### What is shipped instead: the mode row names the preset

The preset's name sits beside the repeat and shuffle icons — `FLAT`, `ROCK`,
`CLASSICAL` — replacing what would otherwise be a static `EQ` label. Same screen
area, but it says *which* preset is on rather than merely that the feature
exists, and being permanent it answers the question a fading toast cannot: a
user who walks away and comes back can see the state without pressing anything.

Nothing else on screen moves. The meter keeps showing the music.

**It works while paused or stopped.** The name updates and the preset takes
