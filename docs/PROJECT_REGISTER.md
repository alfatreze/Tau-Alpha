...
| A-03 | Keep audio-critical state in BRAM; pursue a bounded, framebuffer-priority SDRAM bridge for cold data before any execute-in-place experiment. | Active; Phase 1 contention **Pocket** gate accepted. A-075 proves bridge assembly; A-076 proves final CPU return zero; A-077 proves adapter return zero; A-079 **Pocket** proves owner-mux return zero. The remaining boundary is bridge system-response → mux latch. | [SDRAM architecture](SDRAM_MEMORY_ARCHITECTURE.md), [audit A-075/A-076/A-077/A-079](AUDIT_TRAIL.md), [CPU-window procedure](SDRAM_CPU_WINDOW_DIAGNOSTIC.md) |
| A-080/A-086 | Replace visual-only CPU SDRAM diagnostic transcription with a dedicated, fixed-size APF save record and measured slot lifecycle. | Resolved by A-091: results persist through the `interact.json` channel (Pocket-verified); `0184`/`0188` abandoned. Same previously fitted RBF; no new Quartus gate. | [Current status](CURRENT_STATUS.md), [Issue 019](issues/019-a080-result-log-not-persisted.md), [audit A-086](AUDIT_TRAIL.md) |
| A-081 | Evaluate unused Pocket PSRAM as a separately staged, uncached cold-data path; begin with a 60 MHz asynchronous diagnostic controller and protect the four per-die configuration-register addresses. | Proposed; **design/research** only. No PSRAM RTL or memory-map change is authorised yet. | [PSRAM evaluation plan](PSRAM_EVALUATION_PLAN.md), [audit A-081](AUDIT_TRAIL.md) |
...
| Settings | In-app settings screen (Appearance, Audio, Playback; choice lists, meter previews, Info) | Shipped in v0.2.0-v0.2.2; user-tested on Pocket (A-116..A-137); Advanced, reset, galleries deferred | [settings architecture](SETTINGS_ARCHITECTURE.md), [runtime budget](SETTINGS_RUNTIME_BUDGET.md) |
| Diagnostics | Info page, on-demand tests, stress pump and timed soak | Info page shipped (v0.2.x); tests and stress in the separate Diagnostic Build (A-118, A-125..A-129); safe persistent log and SD speed test still open | [handoff](SESSION_HANDOFF_2026-09-21.md) |
...
| Hardware capacity | Uncached SDRAM CPU data window | **Complete; Pocket validated** on the probe-free seed-2 RBF: coverage, 32-min soak, contention with playback (A-093, A-097, A-100, A-102, A-128, A-129); shipped in v0.2.x | [SDRAM architecture](SDRAM_MEMORY_ARCHITECTURE.md), [audit trail](AUDIT_TRAIL.md) |
| Hardware capacity | Cached SDRAM data window and further cold-workspace migration | First move done: playlist buffers (13 KiB) behind the uncached window (A-105..A-108). Artwork buffers (13 KiB) and the cached window are optional and need a spec (blit engine versus cached window). | [SDRAM architecture](SDRAM_MEMORY_ARCHITECTURE.md), [playlist move](PLAYLIST_SDRAM_MOVE_SPEC.md) |
...
| [018](issues/018-phase2-post-bridge-write-readback.md) | A-065 reaches the SDRAM-domain bridge with `FFFFFFFF` and both controller requests accepted, but later reads still return zero. | Resolved 2026-09-20 by A-093: the adapter double-issued each completed beat, so ACKs carried the previous beat's data. Fixed and **Pocket** verified (183 checks, 0 failures). |
| [019](issues/019-a080-result-log-not-persisted.md) | A-085 reaches `READY D #02`, but its read follows a timed-out flush and remains inconclusive; file is zero. | Resolved 2026-09-19 (A-088 address fix, A-091 interact.json persistence; Pocket-verified). |
