...
### B-150 — Tau Omega now decodes the full TAUD1 QR report, using the screenshots B-133 recovered
**Date:** 2026-09-23
**Evidence:** Continuing the sibling `Tau Omega` session's own thread (B-133 recovered real Check/QR screenshots from the Pocket's `Memories/Screenshots` folder, closing that project's "no real QR fixture" gap). New `tau_core::taud` there is a byte-exact Rust port of this repo's own `tools/decode_tau_suite.py` (`parse_record`/`from_text`): decodes the full TLV record (build identity, SDRAM/PSRAM cycle histograms, the `CT_AUD` playback window, Decode Profile Sweep entries, per-test PASS/FAIL/SKIPPED/N/A including the `CT_BLT` busy-permille special case) and validates the CRC32 trailer, plus a PNG-decode + QR-grid-detection pipeline (`png` + `rqrr` crates) to read it straight from a screenshot file. Verified against 7 of B-133's real fixtures, cross-checked against this repo's own `decode_tau_suite.py --qr` output on the same files (a FULL-profile pass, the real SDRAM-speed FAILURE from B-060, a USER CHECK short run, a STANDARD run with stress tests, and the non-Check now-playing screenshot correctly reported as "no QR found"). Only this repo's own decoder was read as a reference (not copied from) -- the Rust code is an independent implementation matching its documented behaviour.
**Owner instruction:** "next" (in the Tau Omega session), following up on the QR-decoder dependency question B-133 had left open; confirmed the specific dependency choice via `AskUserQuestion` there before adding `base64`/`png`/`rqrr`.
**Not done:** nothing in this repo changed -- the implementation lives entirely in the sibling `Tau Omega` repo (`crates/tau-core/src/taud.rs`, committed and pushed there). This entry exists only per this file's own section 3 rule (log every hardware-evidence-adjacent result here), since the feature consumes this repo's fixtures and reference decoder. No card, VM, or RTL touched.

### B-151 — Tau Omega's engine write paths (sync/package-install/remove) validated against the real mounted Pocket card, then reverted clean
**Date:** 2026-09-23
**Evidence:** Owner ("go ahead") approved real-hardware write validation for the sibling `Tau Omega` project's `sync`/`package`/`remove` engine paths (this repo's own SD-card-write caution -- "ask before any SD-card write... backup/verify/eject" -- applied even though the code under test lives in `Tau Omega`, since it's the same physical card). SHA-256-verified `alfatreze.TAU`/`alfatreze.TAU_DIAGNOSTIC`, `Assets/tau/common/tau.rom` and both `Platforms/tau.*` files before and after every stage. **Sync:** 3 real MP3s from `work/test-music/tau_sdram_wst/common/Controls/` copied into a brand-new `Assets/tauomegasync/common` via `tau-cli plan`/`sync`; index built, read back correctly. **Package install:** the real `Tau Omega/testdata/packages/alfatreze.TAU_0.4.0_2026-09-22.zip` repackaged under an entirely separate platform/core namespace (`tauomegapkg`/`alfatreze.TAU_OMEGA_PKG`, zero path overlap with the real `tau` platform or this card's actual Tau cores) and installed via a throwaway scratch binary linking `tau_core` directly, since no CLI subcommand exists yet for package/remove; `plan_install` confirmed all 15 entries `OnlyLeft` before writing, and `Identical` on a re-plan after. **Remove:** removed the just-installed test core (solo/non-shared-platform case); plan correctly scoped to only the 5 `tauomegapkg` paths, execute reported 28 files / 2,315,518 bytes removed, verified gone. Found one real, harmless edge case only visible on an actual mounted volume: Finder's `._*` AppleDouble junk under `Assets/tauomegapkg/` defeated `plan_remove`'s whole-folder-collapse optimization (expects only the core's own subfolder + `common`), correctly falling back to the two safe per-subfolder deletes instead of guessing -- not a bug, the designed fallback working as intended. All scratch namespaces removed by hand afterward (not by the engine, which had already done its part); card ejected cleanly (`diskutil eject`).
**Owner instruction:** "go ahead," after `AskUserQuestion` confirmed scope: all three write paths, staged low-risk to high-risk, target the real mounted `/Volumes/Pock` under a clearly-scratch namespace only.
**Not done:** mirror-mode delete-on-sync and core-move (backup + two-token deletion confirmation) were not exercised -- today's scope was the three plan→confirm→execute paths already implemented and GUI-wired in `Tau Omega`. Full details and the stray `src/main.rs` mistake (a scratch script accidentally written to this repo's root instead of the scratchpad, caught immediately via `git status` and deleted before it was ever staged) are in `Tau Omega/docs/STATUS_HANDOFF.md`'s own "Real hardware write validation" section.

### B-152 — Tau Omega now discovers screenshots on a card automatically, using the same `Memories/Screenshots/` location B-133 found
**Date:** 2026-09-24
**Evidence:** Owner ("proceed with the automatic screenshot") closed the last open item from the sibling `Tau Omega` session's own implementation-order list. New `tau_core::screenshots::{ScreenshotEntry, list_screenshots}` there lists every file under a card's `Memories/Screenshots/` (the location B-133 identified on this repo's own mounted card), newest first, parsing the Pocket's own `YYYYMMDD_HHMMSS.png` filename shape into a plain timestamp while still listing anything that doesn't match rather than dropping it, and skipping Finder's `._*` junk and non-PNG files; returns an empty list, not an error, for a card that has never taken a screenshot. Wired into a Tauri command and a "Browse screenshots on a card" section on the Settings page, sitting directly above the QR-decode card from B-150 so a found screenshot can be clicked straight into a decode. 3 new engine tests against the real files in `testdata/screenshots/` (newest-first ordering, timestamp parsing, junk-file skipping) plus a synthetic case for a non-conforming filename.
**Owner instruction:** "proceed with the automatic screenshot" (in the Tau Omega session), following up on the one remaining item from the earlier "next recommended implementation order" list.
**Not done:** nothing in this repo changed -- the implementation lives entirely in the sibling `Tau Omega` repo (`crates/tau-core/src/screenshots.rs`, committed and pushed there). Logged here per section 3 since it reuses this repo's own real-hardware finding (`Memories/Screenshots/`) and fixtures. No card, VM, or RTL touched.
