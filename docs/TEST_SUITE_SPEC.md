...
## 11. Decoder stage cost in the Check record (specced 2026-09-22 as B-088; built the same day as B-089)

**Why:** B-086/B-087 measured MP3/FLAC per-stage decode cost (`docs/ARCHITECTURE_ROADMAP.md` section 2) by reading the on-screen `UI_SHOW_DECODE_PROFILE` row off screenshots by hand. That worked but does not scale: it needs a human to photograph a specific second, transcribe two-digit numbers correctly (the FLAC `R0` result took real investigation to confirm as a genuine reading rather than a transcription slip), and there is no machine-readable record for a future kernel-verification run to diff against. This section specs folding the same measurement into the existing Check/QR channel (`docs/TEST_SUITE_SPEC.md` sections 8-10) so a future run is "boot, run Check, read the QR" — no screenshots, no eyeballing.

**New TLV tag, not a reused one.** `SR_T_DECPROF` (u16 x 4: `h_pct`, `i_pct`, `s_pct` for MP3 Huffman/IMDCT/Subband, `r_pct` for FLAC's bit-reader share), added to the enum in `fw/suite_core.h` alongside the existing 12 tags. `SR_T_AUDIO`'s record already has a spare `0u` field (`fw/suite.inc:315`) that could have carried one value, but decode-stage cost and playback-health (underruns/stalls) are different concerns; overloading that field would make `SR_T_AUDIO`'s format depend on a feature it has nothing to do with. A dedicated tag costs nothing extra (TLV, unknown tags already skip cleanly) and keeps the two independent.

**Accumulate over the whole Check run, not one second.** The screen row resets every second by design (good for watching it live, noisy for a single number). The record should instead report each stage's cycles as a percentage of the *entire* run's elapsed real time — computed once, at the end, from totals kept for the run's whole duration.

**Two separate accumulator sets — the modularity decision.** Not "reuse `mp3_huff_cyc`/`flac_res_cyc` and skip the reset when a report is wanted": that couples the screen row's behavior to the report's behavior through shared mutable state, and whoever changes one has to reason about the other. Instead: the existing per-second counters stay exactly as they are, untouched, still feeding only `UI_SHOW_DECODE_PROFILE`; a second, independent set of run-total counters (`mp3_huff_total_cyc`, `mp3_imdct_total_cyc`, `mp3_sub_total_cyc`, `flac_res_total_cyc`, `flac_lpc_total_cyc`, plus an elapsed-real-time counter) accumulates for the Check record and is read exactly once, at report time. `MPROF_ADD`/`PROF_ADD` fan out to both sets at the same call sites already instrumented in B-086; neither `UI_SHOW_DECODE_PROFILE` nor `SR_T_DECPROF` needs to know the other exists, and either can be built alone. A third future consumer (e.g. a soak-test running average) adds a third set at the same call sites rather than a mode flag on the first two.

**A new Diagnostic Build variant, not a permanent addition to the shipped one.** `MP3_PROFILE`/`FLAC_PROFILE` stay off by default, same as today (`player-profile`, B-086) — a new `fw/build.sh` target combines the existing diagnostic stack (`TAU_CHECK` etc.) with `MP3_PROFILE=1 FLAC_PROFILE=1` so this is opt-in and additive, matching every other diagnostic macro in this codebase (`TAU_DIAG_TESTS`, `TAU_CPU_WINDOW_DIAG`, ...); the everyday Diagnostic Build and release stay exactly as they are.

**Also needed:** `tools/decode_tau_suite.py` gains a decoder for `SR_T_DECPROF`; `sim/test_suite.py` gets a fixture exercising it; this section documents the record layout for future reference. Not touching the four-word persist summary (`sp_pack`) — it is already bit-packed at capacity, and this is an occasional diagnostic, not a value that needs to survive every Quit.

**Built (B-089), same design as specced.** `SR_T_DECPROF` (tag 13) in `fw/suite_core.h`; two independent accumulator sets confirmed at the `MPROF_ADD`/`PROF_ADD` call sites (`mp3dec.c`, `fw/flac.c`) — the per-second screen counters untouched, a second set reset when `CT_AUD` (the existing 15 s playback-counter test) starts and read once when it ends; `fw/build.sh player-library-diagnostic-profile` is the opt-in variant (`MP3_PROFILE=1 FLAC_PROFILE=1` added to the normal Diagnostic Build). `tools/decode_tau_suite.py` and `sim/test_suite.py` updated and passing. Not yet run on hardware. Left uncommitted, along with the rest of the still-uncommitted Check/QR feature it extends (see B-089's audit note).

## 12. Decode Profile Sweep: batch decode-cost measurement, one QR for the whole run (spec only, 2026-09-22, from B-090)

**Why:** B-088/B-089 built exactly one measurement window (`CT_AUD`, ~15 s) on whatever track happened to be playing when Check reached that step — a faithful machine-readable replacement for a screenshot, but still one track, one run, one QR per track. The owner's actual ask (2026-09-22): pick a list of files/an album/a playlist, run every entry for an appropriate time, log every result, and get **one** QR at the end covering the whole batch. That is a different feature, not a tweak to `CT_AUD`, and gets its own menu entry rather than stretching Check's pass/fail framing.

**Track selection — the library's current queue, not a new picker.** The library already tracks "the album/playlist currently browsing/playing from." The sweep's input list is exactly that queue: select it the normal way (open an album or playlist in the library browser), then start the sweep from Diagnostics. `mp3-profile/`, `flac-tests/` and `Hyperion/` (B-086/B-087/B-088's test folders) are already indexed as their own albums, so they are sweepable with no new selection UI.

**Per-track window: 15 s capped by track length, matching `CHK_AUDIO_S`.** A track shorter than 15 s (the `flac-tests` vectors, ~15 s each) plays to within a sample of completion; a multi-minute file gets a representative slice. Between tracks: command the next track, wait for playback to actually START (not just "commanded") before resetting the run-total counters -- a load/silence gap must never get counted as decode cost, the same trap `CT_AUD`'s own `chk_at` gating already avoids for a single track.

**Menu placement: a new Diagnostics entry, not a Check mode.** Check is pass/fail; a sweep is a benchmark with no verdict. Keeping them separate keeps both the UI and the record format honest about what each one is. Working name: **Decode Profile Sweep**, `Settings > Diagnostics > Decode Profile Sweep` (same parent as Check in the Diagnostic Build).

**Record: `SR_T_DECPROF` becomes repeatable, one entry per track.** TLV already allows the same tag more than once (the decoder already loops over entries; today it happens to overwrite a single `entries["decprof"]` key per tag, which needs to become a list when more than one `SR_T_DECPROF` is present -- a change to `tools/decode_tau_suite.py`, not the wire format). Per-track payload: `track_idx u8, h_pct u16, i_pct u16, s_pct u16, r_pct u16` = 9 bytes; with the 2-byte TLV header that is **11 bytes per track**. The sweep's own record omits the memory/cold/library/audio tags Check carries (this is not a system health check) -- just `SR_T_BUILD` (~20 bytes with its header) plus N `SR_T_DECPROF` entries, the 4-byte header and 4-byte CRC.

**Size cap: 48 tracks, refuse beyond that.** QR version 20 at level M holds 666 bytes before base64 (section 8). Budget: 666 - 4 (header) - 20 (build) - 4 (CRC) = 638 B / 11 B per track = 58 tracks in the worst case; **48** is the deliberate round number with margin kept for anything the exact byte accounting above is wrong about (level-M error-correction bytes are already inside the 666, not on top of it, but the arithmetic above is worth re-checking against the real QR encoder before this ships). A queue longer than 48 tracks is refused up front with a clear on-screen reason (not silently truncated or paged) -- "one QR at the end" is a promise this cap keeps literal, per the owner's decision; a bigger album needs two sweeps.

**Not built.** Needs: a new `CT_*`-style state machine (mirrors `CT_AUD`'s first-entry/window/next-entry pattern but loops instead of finishing after one track), library-queue introspection (current position, track count, command "next"), the repeatable-tag change to `SR_T_DECPROF`'s consumer in `tools/decode_tau_suite.py`, a new Diagnostics menu row and result page, and host tests for the multi-entry decode. Next free audit id when picked up: B-091.
