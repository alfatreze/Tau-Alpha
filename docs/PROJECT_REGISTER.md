...
| [018](issues/018-phase2-post-bridge-write-readback.md) | A-065 reaches the SDRAM-domain bridge with `FFFFFFFF` and both controller requests accepted, but later reads still return zero. | Open; A-075 Pocket evidence shows A-074's bridge assembled `FFFFFFFF`; A-076 **Pocket** proves the final CPU-facing ACK returns zero. Isolate adapter/mux data from `mp3_soc`'s return selector before any migration decision. |
